...
def main(cfg):
    ...
    for k in range(cfg.folds):
        ...
        print(fold_path / f'fold{k}_test.csv')
        if Path(fold_path / f'fold{k}_test.csv').exists():
            test_patient = pd.read_csv(fold_path / f'fold{k}_test.csv')
            test_idxs = data[data['PATIENT'].isin(test_patient['PATIENT'])].index
            # test dataset (in-domain)
            test_dataset = MILDataset(
                data, test_idxs, [cfg.target], norm=norm_test, clini_info=cfg.clini_info, cls=cfg.cls,
            )
            ...
            if cfg.task == 'binary':
                # 计算类别 0 和类别 1 的样本数量
                num_pos = sum([test_dataset[i][2] for i in range(len(test_dataset))])  # 类别 1 的样本数量
                num_neg = len(test_dataset) - num_pos  # 类别 0 的样本数量
