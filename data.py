...
def get_cohort_df(
    ...
    for target in target_labels:
        ...
        # 检查是否是 TARGET_REG 列，决定转换类型
        if target == 'TARGET_REG':
            df[target] = df[target].map(lambda p: float(p) if p.replace('.', '', 1).isdigit() else label_dict.get(p, p))
        else:
            df[target] = df[target].map(lambda p: int(p) if p.isdigit() else label_dict.get(p, p))
    ...
    h5_df['FILENAME'] = h5_df['slide_path'].apply(lambda p: Path(p).stem)
    ...
    def __getitem__(self, item):
        ...
        if (self.norm == 'histaugan' or
            ...
            if 'feats' in h5_file.keys():
                ...
                # 判定张量大小是否大于 (12000, 768)，如果是则进行截取
                if features.size(0) > 15000:
        ...
        if 'coords' in h5_file.keys():
            # 定义正则表达式模式以提取 level, x 和 y 值
            pattern = re.compile(r'level(\d+)_x(\d+)_y(\d+)_w(\d+)_h(\d+)')
            # 用于存储转换后的结果
            converted_coords = []
            for coord in np.array(h5_file['coords']):
                match = pattern.search(coord.decode('utf-8'))
                if match:
                    ...
                    w = int(match.group(4))
                    h = int(match.group(5))
                    converted_coords.append([level, x, y, w*(level+1), h*(level+1)])
            coords = torch.Tensor(np.array(converted_coords))
        else:
            coords = 0
        ...
        target_label = [self.data[target][self.indices[item]] for target in self.target_labels[0]]
        label = torch.Tensor([target_label[0]]).long().squeeze()

        if self.cls == 'class+reg' or self.cls == 'reg':
            label_reg = torch.Tensor([target_label[1]]).long().squeeze()
            return features, coords, label, label_reg, tiles, patient
        else:
